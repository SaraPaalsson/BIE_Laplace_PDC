...
void vandernewton(const double complex *T, double complex *b, int N) {
  int k, j;
   for (k = 1; k < N; k++) {
    for (j = N - 1; j >= k; j--) {
      b[j] -= T[k - 1] * b[j - 1];
...
void IPmultR(const double complex *in, double complex *out) {
  int i, j, ptr;
...
void specialquadlapl(double *u_specq, double *u_standardq, double *mu, double complex *zDom, double complex *zDrops, double complex *zpDrops, double *wDrops, double complex *panels, int nbr_el_proc) {
  /*
  int i, k, j;
  int furthercheck;
